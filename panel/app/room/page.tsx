...
type State = {
  tiers: {
    warm: { vendors: { name: string; trust?: number; status?: string; purchases?: number }[]; purchases: { vendor?: string; amount_micro?: number }[] };
    cold_count: number;
...
export default function Room() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setLoadErr(true); /* feed keeps local history; tiers show retry state */
    }
  }, []);
  ...
  const refusals = feed.filter((d) => !d.decision.approve).length;
    ...
    <main>
      ...
      <div className="deck">
            ...
            <article key={i} className={`card ${d.decision.approve ? "approve" : "refuse"}`}>
              ...
              <div className="body">
                <div className="reason">
                  {d.request.vendor}:{d.request.sku} · <span className="num">{usd(d.request.amount_micro)}</span> — {d.decision.reason}
                </div>
                ...
                <div className="meta">
                  ...
                  {d.ledger_id && (
                    <a className="tx num" href={`/proof?id=${d.ledger_id}`}>
                      {d.decision.approve ? `tx ${d.payment.tx.slice(0, 22)}…` : "public proof"} ›
                    </a>
                  )}
                </div>
              </div>
        ...
        <section className="zone">
          ...
          {!loadErr && !state && <div className="skeleton" aria-label="loading tiers" />}
          {state && (
            <>
              <div className="tier">
                <h3>warm · vendors <span className="count num">{String(vendors.length)}</span></h3>
              ...
              <div className="tier">
                <h3>cold · journal <span className="count num">{String(state.tiers.cold_count)}</span></h3>
                <div className="rowline">
                  <span className="n">append-only events</span>
                  <span className="d num">{String(purchases.length)} payments</span>
                </div>
              </div>
              <div className="tier">
                <h3>hot · session</h3>
